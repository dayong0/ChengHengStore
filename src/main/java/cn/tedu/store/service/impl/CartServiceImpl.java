...
public class CartServiceImpl implements ICartService {
	...
	public Integer delete(Integer[] cids, Integer uid) {
		...
		Integer[] deleteCids = new Integer[carts.size()];
		for (int i = 0; i < deleteCids.length; i++) {
			deleteCids[i] = carts.get(i).getCid();
		}
		Integer rows = deleteByCids(deleteCids);
		return rows;
	}
	...
	/**
	 * 删除多条购物车数据
	 * @param cids 需要删除的数据的id
	 */
	private Integer deleteByCids(Integer[] cids) {
